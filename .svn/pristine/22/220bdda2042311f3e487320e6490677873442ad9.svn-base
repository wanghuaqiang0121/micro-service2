...
public class SleepConfigController {
	...
	public JsonApi insert(@Validated({ BaseEntity.Insert.class }) @RequestBody SleepConfig sleepConfig, BindingResult result, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
	...
	public JsonApi update(@PathVariable("id") Integer id, @Validated({ BaseEntity.Update.class }) @RequestBody SleepConfig sleepConfig, BindingResult result, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
	...
	public JsonApi getDetail(@PathVariable("id") Integer id, @Validated({ BaseEntity.SelectOne.class }) SleepConfig sleepConfig, BindingResult result, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
		sleepConfig.setId(id);
		Map<String, Object> sleepConfigMap = sleepConfigService.getOne(sleepConfig);
		if (sleepConfigMap != null ) {
	...
	public JsonApi delete(@PathVariable("id") Integer id, @Validated({ BaseEntity.Delete.class }) SleepConfig sleepConfig, BindingResult result, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
		sleepConfig.setId(id);
		/* 检查数据是否存在 */
		Map<String, Object> sleepConfigMap = sleepConfigService.getOne(sleepConfig);
		if (sleepConfigMap == null ) {
	...
	public JsonApi getList(@Validated({ BaseEntity.SelectAll.class })  SleepConfig sleepConfig, BindingResult result, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
