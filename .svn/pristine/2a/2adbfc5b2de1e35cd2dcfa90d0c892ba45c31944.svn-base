...
public class DiagnosisService {
	...
	public Map<String, Object> getBMIDiagramChart(Diagnosis diagnosis) {
		...
		List<Map<String, Object>> list = childrenMeasureMapper.getListByMonthAge(childrenMeasure);
		for (Map<String, Object> userWeightMap : list) {
			float currentWeight =(float) userWeightMap.get("currentWeight");
			float currentHeight =(float) userWeightMap.get("currentHeight");
			//HeightObesityCalculation.div(currentWeight, HeightObesityCalculation.mul(HeightObesityCalculation.div(currentHeight, 100, 2),HeightObesityCalculation.div(currentHeight, 100, 2)), 2);
			// 体重/（身高*身高） 体重单位 kg,身高单位 m
			userWeightMap.put("currentWeight", HeightObesityCalculation.div(currentWeight, HeightObesityCalculation.mul(HeightObesityCalculation.div(currentHeight, 100, 2),HeightObesityCalculation.div(currentHeight, 100, 2)), 2));
	 ...
	 * @description: 头围曲线图
	 */
	public Map<String, Object> getHeadCircumferenceDiagramChart(Diagnosis diagnosis) {

		/* 返回对象 */
		Map<String, Object> map = new HashMap<String, Object>();

		WhoStandardHeadCircumferenceSd whoStandardHeadCircumferenceSd = new WhoStandardHeadCircumferenceSd();
		...
		map.put("subThree", whoStandardHeadCircumferenceSdMapper.getList(whoStandardHeadCircumferenceSd));

		/* 查询-2SD标准线 */
		whoStandardHeadCircumferenceSd.setSd(-2);
		map.put("subTwo", whoStandardHeadCircumferenceSdMapper.getList(whoStandardHeadCircumferenceSd));

		/* 查询-1SD标准线 */
		whoStandardHeadCircumferenceSd.setSd(-1);
		map.put("subOne", whoStandardHeadCircumferenceSdMapper.getList(whoStandardHeadCircumferenceSd));

		/* 查询0SD标准线 */
		whoStandardHeadCircumferenceSd.setSd(0);
		map.put("median", whoStandardHeadCircumferenceSdMapper.getList(whoStandardHeadCircumferenceSd));

		/* 查询1SD标准线 */
		whoStandardHeadCircumferenceSd.setSd(1);
		map.put("plusOne", whoStandardHeadCircumferenceSdMapper.getList(whoStandardHeadCircumferenceSd));

		/* 查询2SD标准线 */
		whoStandardHeadCircumferenceSd.setSd(2);
		map.put("plusTwo", whoStandardHeadCircumferenceSdMapper.getList(whoStandardHeadCircumferenceSd));

		/* 查询3SD标准线 */
		whoStandardHeadCircumferenceSd.setSd(3);
		map.put("plusThree", whoStandardHeadCircumferenceSdMapper.getList(whoStandardHeadCircumferenceSd));
		...
		for (Map<String, Object> userWeightMap : list) {
			...
			WhoStandardHeadCircumferenceSd whoStandardHeadCircumferenceSdModel = new WhoStandardHeadCircumferenceSd();
			whoStandardHeadCircumferenceSdModel.setSex(diagnosis.getSex());
			whoStandardHeadCircumferenceSdModel.setMonthAge((Integer) userWeightMap.get("monthAge"));
			whoStandardHeadCircumferenceSdModel
					.setStandardValue(new BigDecimal(userWeightMap.get("currentWeight").toString()));

			Map<String, Object> whoStandardHeadCircumferenceSdMap = whoStandardHeadCircumferenceSdMapper
					.getHeadCircumferenceSd(whoStandardHeadCircumferenceSdModel);

			Integer sd = (Integer) whoStandardHeadCircumferenceSdMap.get("sd");
			userWeightMap.put("sd", sd);

			if (sd == -3 && ((BigDecimal) whoStandardHeadCircumferenceSdMap.get("standardValue"))
					.compareTo(new BigDecimal(diagnosis.getWeight())) == 1) {
				/* 判断是否小于最低 */
				userWeightMap.put("operator", BaseGlobal.LESS_THAN);
			} else if (sd == 3 && ((BigDecimal) whoStandardHeadCircumferenceSdMap.get("standardValue"))
					.compareTo(new BigDecimal(diagnosis.getWeight())) == -1) {
				userWeightMap.put("operator", BaseGlobal.GREATER_THAN);
			} else {
				userWeightMap.put("operator", BaseGlobal.EQUAL);
	...
	public double getAgeWeight(Diagnosis diagnosis) {
		if (diagnosis.getMonthAge() > 60) {
			...
			whoStandardWeightSdModel.setStandardValue(new BigDecimal(diagnosis.getWeight()));
	 ...
	 * @param weight 体重
	 * @return
	 * @description: 计算bmi：体质指数bmi=体重kg / (身高（m）* 身高（m）)
	 */
