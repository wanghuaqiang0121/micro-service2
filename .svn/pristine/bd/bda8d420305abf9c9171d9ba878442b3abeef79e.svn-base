...
public class AgeOrderController {
	 ...
	 * @description: 修改骨龄评分
	 */
	@RequiresAuthentication(value = { "web-module-bone-age:age-order:update" }, level = Level.OPERATION)
	...
	public JsonApi updateBone(@PathVariable("id") Integer id,
		...
		if(ageOrderService.update(ageOrder) > 0){
			...
			ageOrder.setId((Integer)map.get("ageId"));
	 ...
	 * @description: 生成报告
	 */
	@RequiresAuthentication(value = { "web-module-bone-age:age-order:update-generate-report" }, level = Level.OPERATION )
	 ...
	 * @description: 年龄工单详情
	 */
	@RequiresAuthentication(value = { "web-module-bone-age:age-order:get-one" }, level = Level.OPERATION)
	...
	public JsonApi getOne(@PathVariable("id") Integer id,@Validated({BaseEntity.SelectOne.class })BaseAgeOrder ageOrder,
		...
		if (ageOrderMap != null) {
			...
			ageOrderMap.put("reportDate", now.get(Calendar.YEAR)+"-"+(now.get(Calendar.MONTH) + 1)+"-"+now.get(Calendar.DAY_OF_MONTH));
