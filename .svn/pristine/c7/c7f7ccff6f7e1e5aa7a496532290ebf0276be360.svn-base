...
public class RemoteQuestionController {
	 ...
	 */
	@RequiresAuthentication(level = Level.OPERATION, value = { "web-module-bone-age:remote-question:get-list" })
	@GetMapping(value = { "/remote/question/{id}" })
	public JsonApi getList(@PathVariable("id") Integer id,@Validated({ BaseEntity.SelectAll.class })  RemoteQuestion remoteQuestion, BindingResult result, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
		 ...
		 * @param result
		 * @return
		 * @description: 查询疑问最后一条数据状态
		 */
		@RequiresAuthentication(ignore=true,level = Level.OPERATION, value = { "web-module-bone-age:remote-question:get-one" })
		@GetMapping(value = { "/remote/question/type/{id}" })
		public JsonApi getIsAnswer(@PathVariable("id") Integer id,@Validated({ BaseEntity.SelectOne.class })  RemoteQuestion remoteQuestion, BindingResult result, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
			Map<String, Object> map=new HashMap<>();
			remoteQuestion.setBaseRemoteBoneAgeOrderId(id);
			Map<String, Object> remap=remoteQuestionService.getIsAnswer(remoteQuestion);
			if (remap!=null) {
				int type=Integer.parseInt(remap.get("type").toString());
				if (type==2) {
	 ...
	 */
	@RequiresAuthentication(level=Level.OPERATION, value = { "web-module-bone-age:remote-question:insert-launch" })
	 ...
	 * @description: 回答疑问
	 */
	@RequiresAuthentication(level=Level.OPERATION, value = { "web-module-bone-age:remote-question:insert-answer" })
