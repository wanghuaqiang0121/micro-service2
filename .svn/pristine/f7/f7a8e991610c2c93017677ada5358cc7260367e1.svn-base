...
public class UserController {
	...
	public JsonApi update(@PathVariable("id") Integer id,@Validated({ BaseEntity.Update.class }) @RequestBody User user, BindingResult result,@RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
		...
		/* 是否修改证件 */
		if (user.getCertificateClamps() != null && user.getCertificateClamps().get(0).getCertificateNumber() != null
				&& !"".equals(user.getCertificateClamps().get(0).getCertificateNumber())) {
			if (user.getCertificateClamps().get(0).getCertificateType() == null || "".equals(user.getCertificateClamps().get(0).getCertificateType())) {
				return new JsonApi(ApiCodeEnum.FAIL,Prompt.bundle("user.certificate.is.not.null"));
			}
			UserCertificate userCertificate = user.getCertificateClamps().get(0);
