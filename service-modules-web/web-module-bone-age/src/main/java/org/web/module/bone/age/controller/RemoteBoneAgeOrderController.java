...
public class RemoteBoneAgeOrderController {
	 ...
	 * @description: 远程阅片列表
	 */
	@RequiresAuthentication(level = Level.OPERATION, value = { "web-module-bone-age:remote-bone-age-order:get-list" })
	 ...
	 * @description: 阅片历史
	 */
	@RequiresAuthentication(level = Level.OPERATION, value = { "web-module-bone-age:remote-bone-age-order:get-list-receive" })
	@GetMapping(value = { "/remote/bone/age/order/receive" })
	public JsonApi getListByReceiveOrganizationUserId(@Validated({ RemoteBoneAgeOrder.History.class })  RemoteBoneAgeOrder remoteBoneAgeOrder, BindingResult result,
			@RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token,
		...
		if (jsonApi.getCode() == ApiCodeEnum.OK.getValue()) {
			organizationUserId = Integer.parseInt(jsonApi.getData().toString());
			remoteBoneAgeOrder.setReceiveOrganizationUserId(organizationUserId);
		}else {
			 return new JsonApi(ApiCodeEnum.NOT_FOUND).setMsg(Prompt.bundle("user.id.is.notblank.valid"));
		}
		remoteBoneAgeOrder.setReceiveOrganizationId(organizationId);
		Page<?> page = PageHelper.startPage(remoteBoneAgeOrder.getPage(), remoteBoneAgeOrder.getPageSize());
		List<Map<String, Object>> remoteBoneAgeOrderList = remoteBoneAgeOrderService.getListByReceiveOrganizationUserId(remoteBoneAgeOrder);
		if (remoteBoneAgeOrderList != null && !remoteBoneAgeOrderList.isEmpty()) {
			return new JsonApi(ApiCodeEnum.OK, new MultiLine(page.getTotal(), remoteBoneAgeOrderList));
		}
		return new JsonApi(ApiCodeEnum.NOT_FOUND);
	 ...
	 */
	@RequiresAuthentication(level = Level.OPERATION, value = { "web-module-bone-age:remote-bone-age-order:get-incomplete-list" })
	 ...
	 * @description: 重新发送
	 */
	@RequiresAuthentication(level = Level.OPERATION, value = { "web-module-bone-age:remote-bone-age-order:update" })
	@PutMapping(value = { "/remote/bone/age/order/{id}" })
	public JsonApi update(@PathVariable("id") Integer id, @Validated({ BaseEntity.Update.class }) @RequestBody RemoteBoneAgeOrder remoteBoneAgeOrder, BindingResult result, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
		...
		if (remoteBoneAgeOrderMap == null) {
			...
			if (readFilmRechargeRecordList.get(0).get("lockMoney")!=null) {
				...
				readFilmRechargeRecord.setLockMoney(doctorPrice+lockMoney);
			}else {
	 ...
	 * @description: 催单
	 */
	@RequiresAuthentication(level = Level.OPERATION, value = { "web-module-bone-age:remote-bone-age-order:update-reminder" })
	@PutMapping(value = { "/remote/bone/age/order/reminder/{id}" })
	public JsonApi updateReminder(@PathVariable("id") Integer id, @Validated({ BaseEntity.Update.class }) @RequestBody RemoteBoneAgeOrder remoteBoneAgeOrder, BindingResult result, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
		remoteBoneAgeOrder.setId(id);
		/* 检查数据是否存在 */
		Map<String, Object> remoteBoneAgeOrderMap = remoteBoneAgeOrderService.getOne(remoteBoneAgeOrder);
		if (remoteBoneAgeOrderMap == null) {
	 ...
	 * @description: 禁用/撤回
	 */
	@RequiresAuthentication(level = Level.OPERATION, value = { "web-module-bone-age:remote-bone-age-order:update-disable" })
	...
	public JsonApi updateDisable(@PathVariable("id") Integer id, @Validated({ BaseEntity.Update.class }) @RequestBody RemoteBoneAgeOrder remoteBoneAgeOrder, BindingResult result, 
		...
		if (remoteBoneAgeOrderMap == null) {
			...
			Integer receiveOrganizationUserId=(Integer) remoteBoneAgeOrderMap.get("receiveOrganizationUserId");
			Integer receiveId=(Integer) remoteBoneAgeOrderMap.get("receiveOrganizationId");
			/*医生价格*/
			ReadFilmDoctorPrice readFilmDoctorPrice=new ReadFilmDoctorPrice();
			readFilmDoctorPrice.setDoctorId(receiveOrganizationUserId);
			readFilmDoctorPrice.setOrganizationId(receiveId);
			Map<String, Object> readFilmDoctorPriceMap=readFilmDoctorPriceService.getOne(readFilmDoctorPrice);
			if (readFilmDoctorPriceMap==null) {
				return new JsonApi(ApiCodeEnum.NOT_FOUND);
			}
	 ...
	 * @description: 接单
	 */
	@RequiresAuthentication(level = Level.OPERATION, value = { "web-module-bone-age:remote-bone-age-order:update-receipt" })
	@PutMapping(value = { "/remote/bone/age/order/receipt/{id}" })
	public JsonApi updateReceipt(@PathVariable("id") Integer id, @Validated({ BaseEntity.Update.class }) @RequestBody RemoteBoneAgeOrder remoteBoneAgeOrder, BindingResult result, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
		remoteBoneAgeOrder.setId(id);
		/* 检查数据是否存在 */
		Map<String, Object> remoteBoneAgeOrderMap = remoteBoneAgeOrderService.getOne(remoteBoneAgeOrder);
		if (remoteBoneAgeOrderMap == null) {
	 ...
	 */
	@RequiresAuthentication(level = Level.OPERATION, value = { "web-module-bone-age:remote-bone-age-order:get-statistics-times" })
	 ...
	 */
	@RequiresAuthentication(level = Level.OPERATION, value = { "web-module-bone-age:remote-bone-age-order:get-doctor" })
	@GetMapping(value = { "/remote/bone/age/order/doctors" })
	public JsonApi getDoctors(@Validated({ BaseEntity.SelectAll.class })  RemoteBoneAgeOrder remoteBoneAgeOrder, BindingResult result
			,@RequestHeader(required = true, value = BaseGlobal.ORGANIZATION_ID) Integer organizationId, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
		remoteBoneAgeOrder.setStatus(GlobalEnum.RemoteBoneAgeOrder.COMPLETED.getValue());
		remoteBoneAgeOrder.setLaunchOrganizationId(organizationId);
		Page<?> page = PageHelper.startPage(remoteBoneAgeOrder.getPage(), remoteBoneAgeOrder.getPageSize());
		List<Map<String, Object>> remoteBoneAgeOrderList = remoteBoneAgeOrderService.getDoctors(remoteBoneAgeOrder);
		if (remoteBoneAgeOrderList != null && !remoteBoneAgeOrderList.isEmpty()) {
			return new JsonApi(ApiCodeEnum.OK, new MultiLine(page.getTotal(), remoteBoneAgeOrderList));
		}
		return new JsonApi(ApiCodeEnum.NOT_FOUND);
	 ...
	 * @param organizationId
	 * @return
	 * @description: 查询远程阅片详情
	 */
	@RequiresAuthentication(ignore=true,level = Level.OPERATION, value = { "web-module-bone-age:remote-bone-age-order:get-one" })
	@GetMapping(value = { "/remote/bone/age/order/{id}" })
	public JsonApi getOne(@Validated({ BaseEntity.SelectOne.class })  RemoteBoneAgeOrder remoteBoneAgeOrder, BindingResult result
			,@RequestHeader(required = true, value = BaseGlobal.ORGANIZATION_ID) Integer organizationId, @RequestHeader(required = true, value = BaseGlobal.TOKEN_FLAG) String token) {
		Map<String, Object> remoteBoneAgeOrderMap = remoteBoneAgeOrderService.getOne(remoteBoneAgeOrder);
		if (remoteBoneAgeOrderMap != null ) {
			return new JsonApi(ApiCodeEnum.OK,remoteBoneAgeOrderMap );
		}
		return new JsonApi(ApiCodeEnum.NOT_FOUND);
	}
