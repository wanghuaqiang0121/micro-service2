package org.web.module.height.obesity.intercept;
...
public class UserSecurityInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		if (handler instanceof HandlerMethod) {
			...
			String organizationTeamId = request.getHeader(BaseGlobal.ORGANIZATION_TEAM_ID);
			/* 获取模块ID */
			...
			if (authentication.ignore()) {
				...
				RedisSession session = redisCacheManager.getSession(BaseGlobal.CACHE_ORGANIZATION_USER, token);
				/* 登录信息不为空证明已登录，则放行 */
				if (session != null) {
					return true;
				}
				...
				case ROLE:
					/* 获取角色列表 */
					JsonApi roleJson = organizationUserRoleService.getOrganizationOrganizationTeamRoles(1, 1000, token, Integer.valueOf(organizationTeamId));
					if (roleJson.getCode() != ApiCodeEnum.OK.getValue()) {
						/* 发生错误返回错误消息 */
						response.getWriter().write(JSON.toJSONString(new JsonApi(ApiCodeEnum.FORBIDDEN)));
						return false;
					}
					List<Map<String, Object>> roleList = roleJson.getRows();
				...
				case PERMISSION:
					/* 获取权限列表 */
					JsonApi permissionJson =  organizationUserRoleService.getOrganizationOrganizationTeamPermissions(1, 1000, token, Integer.valueOf(organizationTeamId));
					if (permissionJson.getCode() != ApiCodeEnum.OK.getValue()) {
						/* 发生错误返回错误消息 */
						response.getWriter().write(JSON.toJSONString(new JsonApi(ApiCodeEnum.FORBIDDEN)));
						return false;
					}
					List<Map<String, Object>> permissionList = permissionJson.getRows();
				...
				case OPERATION:
					/* 获取操作列表 */
					JsonApi operationJson =  organizationUserRoleService.getOrganizationOrganizationTeamOperations(1, 1000, token, Integer.valueOf(organizationTeamId));
					...
					//List<Map<String, Object>> operationList = operationJson.getRows();
