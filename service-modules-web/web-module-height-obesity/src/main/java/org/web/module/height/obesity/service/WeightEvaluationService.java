...
public class WeightEvaluationService {
	...
	public Map<String, Object> getObesityEvaluation(Diagnosis diagnosis) {
		...
		if (diagnosis.getChildrenMaternity() != null && diagnosis.getChildrenMaternity().get("birthHeight") != null
			...
			bornBmi = diagnosisService.getBMI(// 根据用户出生身高，体重计算BMI出生值
			...
			whoStandardHeightWeightSd.setHeight(new BigDecimal(bornHeight));
		...
		if (perChildrenMeasureMap != null) {
			// 上次BMI
			double lastBmi = diagnosisService.getBMI(((Float) perChildrenMeasureMap.get("currentHeight")).doubleValue(),
					((Float) perChildrenMeasureMap.get("currentWeight")).doubleValue());
			lastHeight = ((Float) perChildrenMeasureMap.get("currentHeight")).doubleValue();
			lastWeight = ((Float) perChildrenMeasureMap.get("currentWeight")).doubleValue();
		...
		/* 查询用户当前别体重 */
		double curentBmi = diagnosisService.getBMI(// 根据用户当前身高，体重计算BMI当前值
				diagnosis.getHeight(),
				diagnosis.getWeight());
		...
		if (((diagnosis.getEndMonthAge() <= 36 && diagnosis.getWeight() < 2.5) || (diagnosis.getEndMonthAge() <= 36 && diagnosis.getBirthGestational() != null && diagnosis.getBirthGestational() < 37)) && curentWeight != null) {
			if (curentWeight >= curentSub2SdValue && curentWeight < curentSub1SdValue) {// 1.1:  M-2SD≤当前体重<M-1SD（正常偏低）
				evaluation = String.format(BaseGlobal.WEIGHT_OBESITY_EVALUATION_ONE, name, diagnosis.getMonthAge() , "-2SD", name);
			}else if (curentWeight >= curentSub1SdValue && curentWeight < curentPlus1SdValue ){// 1.2：  M-1SD≤当前体重<M+1SD
				evaluation = String.format(BaseGlobal.WEIGHT_OBESITY_EVALUATION_TWO, name, diagnosis.getMonthAge() , "-1SD", name);
			}else if (curentWeight >= curentPlus1SdValue && curentWeight < curentPlus2SdValue) {// 1.3： M+1SD≤当前体重<M+2SD（超重体型）
				evaluation = String.format(BaseGlobal.WEIGHT_OBESITY_EVALUATION_ONE, name, diagnosis.getMonthAge() , "+1SD", name);
				weightWarning = "孩子体型超重";
			}else if ( (curentWeight >= curentSub3SdValue && curentWeight < curentSub2SdValue) && 
						( (bornWeight >= bornSub3SdValue && bornWeight < bornSub2SdValue) || (lastWeight >= lastSub3SdValue && lastWeight < lastSub2SdValue) )) {
				// 1.4： M-3SD≤当前体重<M-2SD 且 [ M-3SD≤出生时||(上一次)体重<M-2SD ]  (中度消瘦)
				evaluation = String.format(BaseGlobal.WEIGHT_OBESITY_EVALUATION_THREE, name, diagnosis.getMonthAge() , name,"%","%","%");
				weightWarning = "孩子体型中度消瘦";
				
			}// 1.41M-3SD≤当前体重<M-2SD(中度消瘦)  且  M-3SD>出生时||(上一次)(重度消瘦)
			else if((curentWeight >= curentSub3SdValue && curentWeight < curentSub2SdValue) && 
					((bornWeight < bornSub3SdValue) || (lastWeight <  lastSub3SdValue))){
				evaluation = String.format(BaseGlobal.WEIGHT_OBESITY_EVALUATION_FOUR_ONE, name, diagnosis.getMonthAge() , name,"%","%","%");
				weightWarning = "孩子体型中度消瘦";
				...
				evaluation = String.format(BaseGlobal.WEIGHT_OBESITY_EVALUATION_FOUR, name, diagnosis.getMonthAge() , name,"%","%","%");
				...
				weightWarning = "孩子体型重度消瘦";
				evaluation = String.format(BaseGlobal.WEIGHT_OBESITY_EVALUATION_FIVE, name, diagnosis.getMonthAge() , name,"%","%","%");
		...
		/*
		 * 腹围评价
		 * 腹围>身高的一半,XXX,腹围超过身高的一半为腹型肥胖
		 * 腹围≤身高的一半，XXX,腹围正常
		 * */
		if (childrenMeasureOneMap != null) {
			// 腹围 身高 不为空
			if (childrenMeasureOneMap.get("abdomenCircumference") != null && childrenMeasureOneMap.get("currentHeight") != null) {
				// 腹围
				float abdomenCircumference = (float) childrenMeasureOneMap.get("abdomenCircumference");
				// 身高
				float currentHeight = (float) childrenMeasureOneMap.get("currentHeight");
				if (abdomenCircumference > (currentHeight/2) ) {//腹围>身高的一半,XXX,腹围超过身高的一半为腹型肥胖
					map.put("abdominalPerimeter", ""+name+",腹围超过身高的一半为腹型肥胖");
				}else{//腹围≤身高的一半，XXX,腹围正常
					map.put("abdominalPerimeter", ""+name+",腹围正常");
				}
			}else{
				map.put("abdominalPerimeter", null);
			}
		}
